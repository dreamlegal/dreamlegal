...
export async function POST(req: NextRequest) {
  try {
    const { action, badgeId, productId, text, borderColor, displayOrder } = await req.json();

    // CREATE BADGE
    if (action === "create") {
      if (!productId || !text || !borderColor) {
        return NextResponse.json(
          { error: "Product ID, text, and border color are required" },
          { status: 400 }
        );
      }
