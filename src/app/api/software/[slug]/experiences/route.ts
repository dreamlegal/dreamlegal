...
// POST - Create or update user experience WITH TRACKING
export async function POST(
  request: NextRequest,
  { params }: { params: { slug: string } }
) {
  try {
    ...
    const existingExperience = await prisma.userExperience.findUnique({
      where: {
        userId_productId: {
          userId,
          productId: software.id
    ...
    const isNewReview = !existingExperience;
    ...
    // Track review activity ONLY if it's a new review
    if (isNewReview) {
      try {
        ...
        console.error('Failed to track review activity:', trackError);
