...
function UserLoginContent() {
  ...
  const handleProviderSignIn = async (provider: 'google' | 'linkedin') => {
    ...
    // Store returnUrl in multiple places
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100">
          <div className="space-y-6">
            ...
            <div className="text-center text-[#334155] mt-8">
              Don't have an account?{' '}
              <Link
                href={`/auth/new/user/signup${returnUrl !== '/legal-professionals/dashboard' ? `?returnUrl=${encodeURIComponent(returnUrl)}` : ''}`}
                className="text-[#7cc6ee] hover:underline font-medium"
              >
                Sign up
              </Link>
