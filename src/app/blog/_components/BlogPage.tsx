...
import { useParams, useRouter } from 'next/navigation';
import { motion, AnimatePresence } from 'framer-motion';
import { ArrowLeft, Calendar, Clock, List, Share2, Facebook, Twitter, Linkedin, Copy, CheckCircle, ChevronDown, ExternalLink, Lock } from 'lucide-react';
import { useNewAuth } from '@/context/NewAuthContext';
import FinalSection from "@/app/(home)/category/_components/FinalSection"

// Import tracking utilities
import { canAccessBlog, trackBlogView, getRemainingBlogViews } from '@/lib/access-tracking';

// Import the voting hook
import { useVoting } from '@/hooks/useVoting.js';
...
export default function BlogPage({ slug }: { slug: string }) {
  ...
  // CHECK ACCESS BEFORE FETCHING BLOG
  useEffect(() => {
    const checkAccess = () => {
      const canAccess = canAccessBlog(slug, isAuthenticated);
      const remaining = getRemainingBlogViews(isAuthenticated);
  ...
  useEffect(() => {
    ...
    async function fetchBlog() {
      try {
        ...
        if (hasAccess) {
          trackBlogView(slug, isAuthenticated);
          // Update remaining count
          setRemainingViews(getRemainingBlogViews(isAuthenticated));
        }
    ...
    fetchBlog();
  }, [slug, router, hasAccess, isAuthenticated, accessChecked]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50/50 to-white">
        <div className="max-w-4xl mx-auto p-6 pt-24">
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="bg-white rounded-2xl shadow-xl p-8 md:p-12 text-center border-2 border-[#7cc6ee]"
          >
            ...
            <p className="text-lg text-gray-600 mb-6">
              You've read your 3 free articles this month. Sign in to get unlimited access to all our content!
            </p>
            ...
            <button
              ...
              className="bg-[#7cc6ee] hover:bg-[#1e2556] text-white font-bold py-4 px-8 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 text-lg"
            >
              Sign In for Free
            </button>
            ...
            <Link 
              href="/blog" 
              className="mt-4 inline-flex items-center text-[#7cc6ee] hover:text-[#1e2556] font-medium"
            >
              <ArrowLeft size={16} className="mr-2" />
  ...
  // ========== REST OF YOUR EXISTING BLOG PAGE CODE ==========
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50/50 to-white">
      ...
      {/* REMAINING VIEWS BANNER - Only show if not authenticated and has limited views */}
      {!isAuthenticated && remainingViews > 0 && remainingViews <= 2 && (
        <div className="fixed top-16 left-0 right-0 z-50 bg-gradient-to-r from-yellow-400 to-orange-500 text-white py-3 px-4 shadow-lg">
          <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-2">
            <p className="font-semibold text-sm md:text-base">
              ⚠️ {remainingViews === 1 ? 'This is your last free article this month!' : `You have ${remainingViews} free articles left this month.`}
            </p>
            <button
              onClick={showAuthModal}
      ...
      <div ref={contentRef} className={`max-w-7xl mx-auto p-3 md:p-6 pb-8 md:pb-16 ${!isAuthenticated && remainingViews <= 2 ? 'pt-28 md:pt-32' : 'pt-16 md:pt-24'}`}>
        ...
        <a href="https://www.zoho.com/contracts/ebooks/state-of-contract-management.html" target="_blank" rel="noopener noreferrer">
          <img
            src="/images/banner.jpg"
            alt="Resources Banner"
            className="w-full h-full object-cover cursor-pointer mb-6"
          />
        </a>

        {/* REST OF YOUR BLOG CONTENT - Keep everything else the same */}
        <div className="flex flex-col lg:flex-row gap-4 md:gap-8">
          ...
          >
            {blog && (
              <>
                ...
                {/* Share buttons (mobile - top) - Keep your existing mobile share section */}
                ...
                {/* Keep rest of your existing sections: References, etc. */}
              </>
            )}
          </motion.div>
          <motion.div 
