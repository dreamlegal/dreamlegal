...
import {
  ...
  Minus, CheckSquare, Undo, Redo, Eraser, MoreHorizontal, ChevronDown,
  Search, Package, ExternalLink, Image as ImageIcon, Table as TableIcon,
  Upload, AlertTriangle, Users, User
} from 'lucide-react';
...
// NEW: Contributor Search Modal Component
const ContributorSearchModal = ({ onClose, onInsert }) => {
  const [searchQuery, setSearchQuery] = useState('');
  ...
  const searchContributors = useCallback(async (query) => {
    if (!query || query.trim().length < 2) {
      setContributors([]);
      return;
    }
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/users/search?q=${encodeURIComponent(query)}&type=user`);
      const data = await response.json();
      setContributors(data.users || []);
    } catch (error) {
      console.error('Contributors search error:', error);
      setContributors([]);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timeoutId);
  }, [searchQuery, searchContributors]);

  const handleContributorSelect = (contributor) => {
    setSelectedContributors(prev => {
      const isSelected = prev.some(c => c.id === contributor.id);
      if (isSelected) {
        return prev.filter(c => c.id !== contributor.id);
      } else {
      ...
      >
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            placeholder="Search for contributors by name, email, or designation..."
            autoFocus
          />
        </div>

        {/* Selected Contributors Preview */}
        {selectedContributors.length > 0 && (
          <div className="mb-4 p-3 bg-white rounded-lg border" style={{ borderColor: '#7cc6ee' }}>
        ...
        <div className="max-h-64 overflow-y-auto mb-4">
          {isLoading && (
            <div className="text-center py-4">
              <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 mx-auto" style={{ borderColor: '#1e2556' }}></div>
              <p className="mt-2" style={{ color: '#334155' }}>Searching contributors...</p>
            </div>
          )}

          {!isLoading && searchQuery.length >= 2 && contributors.length === 0 && (
            <div className="text-center py-4">
              <Users size={48} className="mx-auto mb-2" style={{ color: '#334155' }} />
              <p style={{ color: '#334155' }}>No contributors found for "{searchQuery}"</p>
            </div>
          )}
              ...
              {contributors.map((contributor) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      borderColor: isSelected ? '#1e2556' : '#e5e7eb'
                    }}
                  >
                    <img
                      src={contributor.profilePicture || '/default-avatar.png'}
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg transition text-white flex items-center gap-2 disabled:opacity-50"
            style={{ backgroundColor: selectedContributors.length > 0 ? '#1e2556' : '#334155' }}
            onMouseEnter={(e) => {
              if (selectedContributors.length > 0) e.target.style.backgroundColor = '#7cc6ee';
            }}
            onMouseLeave={(e) => {
              if (selectedContributors.length > 0) e.target.style.backgroundColor = '#1e2556';
            }}
          >
            <Users size={16} />
            Add Contributors ({selectedContributors.length})
          </button>
        </div>
      </div>
    </div>
...
// NEW: Custom Contributor Extension for TipTap
const ContributorWidget = Node.create({
  name: 'contributorWidget',
  ...
  parseHTML() {
    return [
      {
        tag: 'div[data-contributor-widget]',
        getAttrs: (element) => {
          const contributorsData = element.getAttribute('data-contributors');
          return {
            contributors: contributorsData ? JSON.parse(contributorsData) : [],
          };
        },
      },
    ];
  ...
  renderHTML({ HTMLAttributes }) {
    const contributors = HTMLAttributes.contributors || [];
    
    if (contributors.length === 0) {
      return [
        'div',
        {
          'data-contributor-widget': true,
          'data-contributors': JSON.stringify([]),
          class: 'contributor-widget-empty',
          style: 'display: none;'
        }
      ];
    }
    ...
    return [
      'div',
      {
        ...
        class: 'contributor-widget-compact',
        style: 'margin: 1rem 0; padding: 0.75rem 1rem; border-radius: 0.5rem; border: 1px solid #7cc6ee; background-color: #f5f7fa; display: inline-block; max-width: 100%;'
      },
      [
        'div',
        {
        ...
        [
          'svg',
          {
            ...
            stroke: 'currentColor',
            'stroke-width': '2',
            style: 'color: #1e2556; flex-shrink: 0;'
          },
          [
            'path',
            {
        ...
        [
          'span',
          {
            style: 'font-size: 0.875rem; font-weight: 600; color: #1e2556;'
          },
      ...
      [
        ...
        ...contributors.map(contributor => [
          'div',
          {
            class: 'contributor-card-compact',
            style: 'display: inline-flex; align-items: center; gap: 0.375rem; padding: 0.375rem 0.5rem; background-color: white; border-radius: 1rem; border: 1px solid #7cc6ee; white-space: nowrap; max-width: 200px;'
          },
          [
            'img',
            {
              src: contributor.profilePicture || '/default-avatar.png',
              alt: contributor.name,
              style: 'width: 1.5rem; height: 1.5rem; object-fit: cover; border-radius: 50%; border: 1px solid #7cc6ee; flex-shrink: 0;',
          ...
          [
            'span',
            {
              style: 'font-size: 0.75rem; font-weight: 500; color: #1e2556; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; min-width: 0;'
            },
            contributor.name
          ]
        ])
  ...
  addNodeView() {
    return ({ node }) => {
      const div = document.createElement('div');
      div.className = 'contributor-widget-compact';
      
      const contributors = node.attrs.contributors || [];
      ...
      div.innerHTML = `
        <div class="contributor-widget-header">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="contributor-widget-list">
          ${contributors.map(contributor => `
            <div class="contributor-card-compact">
              <img 
                src="${contributor.profilePicture || '/default-avatar.png'}" 
                alt="${contributor.name}" 
                class="contributor-avatar-compact"
                onerror="this.src='/default-avatar.png'"
              >
              <span class="contributor-name-compact">${contributor.name}</span>
            </div>
          `).join('')}
        </div>
      `;
...
// Enhanced toolbar with advanced formatting options + Contributors
const EditorToolbar = ({ editor, onImageAdd, onExtractToc }) => {
  const [showTextColorPicker, setShowTextColorPicker] = useState(false);
  const [showHighlightPicker, setShowHighlightPicker] = useState(false);
  const [showAdvancedTools, setShowAdvancedTools] = useState(false);
  ...
  // NEW: Handle contributor insertion
  const handleContributorInsert = (contributors) => {
    // Find existing contributor widget or create new one
    const existingWidget = editor.state.doc.descendants((node, pos) => {
      if (node.type.name === 'contributorWidget') {
        // Update existing widget with new contributors
        const existingContributors = node.attrs.contributors || [];
        const newContributors = contributors.filter(
          contributor => !existingContributors.some(existing => existing.id === contributor.id)
        );
        ...
        editor.chain()
          .focus()
          .command(({ tr }) => {
            tr.setNodeMarkup(pos, undefined, { contributors: updatedContributors });
            return true;
          })
          .run();
    ...
    // If no existing widget found, create a new one at the end
    if (!existingWidget) {
      editor.chain()
        .focus()
        .insertContentAt(editor.state.doc.content.size, {
          type: 'contributorWidget',
          attrs: {
  ...
  return (
    <div className="border-b sticky top-4 bg-white z-10 shadow-sm rounded-t-lg">
      ...
      {/* NEW: Contributor Search Modal */}
      {showContributorSearch && (
        <ContributorSearchModal
          onClose={() => setShowContributorSearch(false)}
          onInsert={handleContributorInsert}
        />
      )}
    </div>
  );
...
// Enhanced CSS for editor styling with brand colors + image + table support + Contributors
const customStyles = `
.ProseMirror {
  min-height: 400px;
  border-radius: 0.5rem;
...
/* NEW: Contributor Widget Styling - Compact Version */
.contributor-widget-compact {
  margin: 1rem 0;
  padding: 0.75rem 1rem;
  border-radius: 0.5rem;
  border: 1px solid #7cc6ee;
...
.contributor-widget-compact:hover {
  border-color: #1e2556;
  background-color: #ffffff;
  box-shadow: 0 2px 8px rgba(30, 37, 86, 0.1);
}

.contributor-widget-header {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  margin-bottom: 0.5rem;
...
.contributor-widget-header svg {
  color: #1e2556;
  flex-shrink: 0;
}

.contributor-widget-header span {
  font-size: 0.875rem;
  font-weight: 600;
  color: #1e2556;
}

.contributor-widget-list {
  display: flex;
  flex-wrap: wrap;
  gap: 0.5rem;
}
...
.contributor-card-compact:hover {
  border-color: #1e2556;
  box-shadow: 0 1px 4px rgba(30, 37, 86, 0.15);
}

.contributor-avatar-compact {
  width: 1.5rem;
  height: 1.5rem;
  object-fit: cover;
  border-radius: 50%;
...
const TipTapEditor = ({
  ...
  // Initialize editor with all required extensions INCLUDING 5 HEADING LEVELS + Contributors
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        history: {
      ...
      ProductWidget,
      ContributorWidget, // NEW: Add contributor widget
    ],
    content: initialContent,
    onUpdate: ({ editor }) => {
      // FIXED: Extract and update TOC on every content change
