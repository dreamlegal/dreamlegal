...
const MenuItem = ({ icon: Icon, text, path, subItems, subItemPaths, isActive, onClick, useWindowLocation = false }) => {
  const [isOpen, setIsOpen] = useState(false);
  const router = useRouter();
  const pathname = usePathname();
  ...
  const handleClick = () => {
    if (subItems) {
      ...
      if (useWindowLocation) {
        // Replace current page using window.location.href
...
const Sidebar = ({ isOpen, setIsOpen, isMobile }) => {
  ...
  const menuItems = [
    ...
    { 
      ...
      text: 'Prompts', 
      path: '/prompts',
      useWindowLocation: true  // This will use window.location.href instead of router.push
    },
  ];
