...
const RfpDisplayEdit = ({ rfpId }) => {
  ...
  const hasVendors = rfpData && rfpData.vendors && rfpData.vendors.length > 0;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 space-y-4 lg:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 w-full lg:w-auto">
            ...
            {hasVendors ? (
  <Link
    href={`/rfp/${rfpId}/vendors`}
    className="flex items-center justify-center px-6 py-3 rounded-lg font-medium transition-all duration-200 bg-[#1e2556] text-white hover:bg-opacity-90"
  >
    <Target className="w-5 h-5 mr-2" />
    View Matched Vendors ({rfpData.vendors.length})
  </Link>
) : (
  <button
    onClick={handleVendorMatching}
