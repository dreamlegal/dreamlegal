...
const AllProducts = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await fetch("/api/vendor/legal-software", {
        method: "POST",
        headers: {
  ...
  const handleProductUpdated = (updatedProduct) => {
    ...
    showAlert("Premium information has been successfully updated", "success");
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="px-6 py-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Your Legal Software Products</h1>
              <p className="mt-1 text-gray-600">Manage and monitor all your legal software products</p>
            </div>
      ...
      {!loading && !error && (!filteredProducts || filteredProducts.length === 0) && (
        <div className="flex flex-col items-center justify-center p-8 text-center bg-white rounded-xl border border-gray-200 shadow-sm">
          ...
          <p className="text-gray-600">
            {searchTerm ? 'Try adjusting your search terms.' : 'Start by adding your first legal software product.'}
          </p>
        </div>
      )}
