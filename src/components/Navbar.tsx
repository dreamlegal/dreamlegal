...
const Navbar = () => {
  ...
  // Updated navItems - Resources now has dropdown
  const navItems = [
    {
      title: 'Home',
      icon: <Home size={16} />,
              ...
              <div className="hidden md:flex items-center space-x-6">
                {navItems.map((item, index) => (
                  <div key={index} className="relative group">
                    ...
                    {item.hasDropdown && (
                      <div 
                        className={`absolute left-0 mt-1 ${item.isTwoColumn ? 'w-[680px]' : 'w-80'} transition-all duration-200 transform ${
                          activeDropdown === index 
                            ? 'opacity-100 visible translate-y-0' 
                            : 'opacity-0 invisible translate-y-2'
                        }`}
                      ...
                      >
                        <div className="rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.12)] bg-white border border-gray-100 overflow-hidden">
                          <div className={`py-2 px-2 ${item.isTwoColumn ? 'grid grid-cols-2 gap-1' : ''}`}>
                            {item.dropdownItems?.map((dropdownItem, idx) => (
                              <a
                                key={idx}
                                href={dropdownItem.href}
