...
// components/ProductCompareCard.js
import React from "react";
import { IoIosArrowRoundForward } from "react-icons/io";

function ProductCompareCard({ name, slug, logoUrl }) {
  return (
    <div className="bg-white px-5 py-4 border shadow rounded-2xl w-full md:w-[300px] group hover:border-primary1 transition-all duration-200 hover:translate-y-1 hover:cursor-pointer">
      <div className="flex gap-2 items-center">
        <img
        ...
        />
        <div className="flex flex-col">
          <h3 className="text-base font-bold ml-3">{name}</h3>
          <a
            href={`/product/${slug}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex gap-2 rounded-full text-primary1 font-bold px-6 py-3 text-xs transition-all w-fit items-center group-hover:bg-primary2 group-hover:gap-4"
          >
            View Product
            <IoIosArrowRoundForward className="text-xl" />
          </a>
