...
// Modified schema with all validations made optional
const PricingSchema = z.object({
  freeTrial: z.enum(["Yes", "No"]).optional(),
  freeVersion: z.enum(["Yes", "No"]).optional(),
  timePeriod: z.union([z.string(), z.null()]).optional(),
  pricingModel: z.array(z.enum(["Annual Fee", "Monthly subscription", "Volume based"])).optional(),
  contractPeriod: z.union([z.string(), z.null()]).optional(),
  fixedPricing: z.boolean().optional(),
  pricingParams: z.string().optional(),
  pricingFields: z.array(
    z.object({
...
const PricingForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Always succeeds since all fields are optional
    const result = PricingSchema.safeParse(formData);
    
    // No validation errors to handle now
    setErrors({});
