...
function UserProfile({ data, onEditClick, onChangePassword }: any) {
  ...
  const renderArrayField = (label: string, items: string[] | null) => {
  ...
  const ProfileContent = () => (
    <ul className={`mt-2 pb-2 space-y-2 transition-all duration-200 ${
      !CompDetails ? "hidden" : ""
    }`}>
      <li className="grid grid-cols-1 pr-5">
        <p className="text-sm text-slate-500">Email</p>
        <p className="text-sm text-gray-900 truncate">{AccountDetails?.email}</p>
      </li>
      ...
      <li className="grid grid-cols-1 pr-5">
        <p className="text-sm text-slate-500">Organisation Name</p>
        <p className="text-sm text-gray-900">{profile?.CompanyAddress}</p>
      </li>
      ...
      {profile?.CompanyEmail && (
        <li className="grid grid-cols-1 pr-5">
          ...
          <p className="text-sm text-gray-900">{profile.CompanyEmail}</p>
        </li>
      )}
      ...
      {renderArrayField("Existing Tools", profile?.ExistingTools)}
      {renderArrayField("Goals", profile?.Goals)}
      {renderArrayField("Industry", profile?.Industry)}
      {renderArrayField("Practice Areas", profile?.PracticeArea)}
      {renderArrayField("Primary Languages", profile?.PrimaryLanguage)}
      {renderArrayField("Work Types", profile?.WorkType)}
