...
const protectedPaths = {
  ...
  '/tech-vendor/dashboard': ['vendor'],
  '/legal-professionals/dashboard': ['user'],
  '/onboard': ['user'],
...
export async function middleware(request: NextRequest) {
  try {
    ...
    if (path === '/tech-vendor/dashboard') {
      return NextResponse.redirect(new URL('/tech-vendor/dashboard/all_products', request.url));
    }
    if (path === '/legal-professionals/dashboard') {
      return NextResponse.redirect(new URL('/legal-professionals/dashboard/profile', request.url));
    }
    ...
    try {
      ...
      if (!isAllowed) {
        ...
        if (userType === 'vendor') {
          return NextResponse.redirect(new URL('/tech-vendor/dashboard', request.url));
        }
        // If user tries to access vendor dashboard
        if (userType === 'user') {
          return NextResponse.redirect(new URL('/legal-professionals/dashboard', request.url));
        }
      }
...
export const config = {
  matcher: [
    '/tech-vendor/dashboard/:path*',
    '/legal-professionals/dashboard/:path*',
    '/onboard',
    '/community',
